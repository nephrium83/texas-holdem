...
class Holdem:
    ...
    def _mp_after_deal(self) -> None:
        """Host: after dealing, send hole cards (plaintext harness path)
        and broadcast state."""
        e    = self.engine
        sess = self._mp_session
        ...
        for i, cid in enumerate(seat_order):
            ...
            if sp and not sp.is_host and i < len(e.players):
                ...
                # Legacy trusted-host harness path: plaintext deal. The
                # encrypted variant died with the retired commit-reveal
                # shuffle; real multiplayer is the trustless mental-poker
                # deal in the hostless session path.
    ...
    def deal(self):
        ...
        # Deal directly with the RNG-shuffled deck: solo play, and the
        # legacy trusted-host Tkinter harness (its commit-reveal shuffle
        # is retired; the trustless deal lives in the hostless session).
